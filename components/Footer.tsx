...
const Footer: React.FC = () => {
  return (
    <footer className="bg-stone-50 text-stone-600 pt-32 pb-16 relative overflow-hidden border-t border-stone-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-16 lg:gap-24 mb-24">
            ...
            <div className="flex gap-4">
              {[
                ...
                <a 
                  ...
                  className="w-12 h-12 rounded-full border border-stone-200 flex items-center justify-center hover:bg-orange-600 hover:text-white hover:border-orange-600 transition-all duration-500 group"
                  title={social.label}
                >
          ...
          <div className="lg:col-span-2 space-y-10">
            <h3 className="text-stone-900 font-serif font-bold text-xl">Explore</h3>
            <ul className="space-y-4 text-sm font-light">
              {[
                ...
                { label: 'Advaita Store', path: '/advaita' },
                { label: 'Join Us', path: '/volunteer' },
                { label: 'Donate Now', path: '/donate' }
              ].map((link, i) => (
                <li key={i}>
                  <Link to={link.path} className="hover:text-orange-600 transition-colors flex items-center gap-2 group">
                    <span className="w-0 h-px bg-orange-600 group-hover:w-4 transition-all duration-500"></span>
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          {/* Utility Links */}
          <div className="lg:col-span-2 space-y-10">
            <h3 className="text-stone-900 font-serif font-bold text-xl">Resources</h3>
            <ul className="space-y-4 text-sm font-light">
              {[
          ...
          {/* Contact Info */}
          <div className="lg:col-span-4 space-y-10">
            <h3 className="text-stone-900 font-serif font-bold text-xl">Get in Touch</h3>
            <ul className="space-y-8 text-sm font-light">
              <li className="flex gap-5">
                <MapPin size={24} className="text-orange-600 shrink-0" />
                <div className="space-y-2">
                  <span className="text-stone-900 font-bold text-[10px] uppercase tracking-widest block">Working Address</span>
              ...
              <li className="flex gap-5">
                ...
                <div className="space-y-2">
                  <span className="text-stone-900 font-bold text-[10px] uppercase tracking-widest block">Email Us</span>
                  <a href={`mailto:${CONTACT_INFO.emails[0]}`} className="text-stone-500 hover:text-orange-600 transition-colors break-all">{CONTACT_INFO.emails[0]}</a>
                </div>
              </li>
              <li className="flex gap-5">
                <Phone size={24} className="text-orange-600 shrink-0" />
                <div className="space-y-2">
                  <span className="text-stone-900 font-bold text-[10px] uppercase tracking-widest block">Call Us</span>
                  <a href={`tel:${CONTACT_INFO.phones[0]}`} className="text-stone-500 hover:text-orange-600 transition-colors">{CONTACT_INFO.phones[0]}</a>
                </div>
              </li>
            </ul>
          </div>
        ...
        {/* Bottom Bar */}
        <div className="pt-16 border-t border-stone-200 flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex items-center gap-8 text-[10px] font-bold uppercase tracking-[0.2em] text-stone-400">
            <span>Section 8 NGO</span>
            <span className="w-1 h-1 bg-stone-200 rounded-full"></span>
            <span>Registered in Uttarakhand</span>
            <span className="w-1 h-1 bg-stone-200 rounded-full"></span>
            <Link to="/documents" className="hover:text-stone-900 transition-colors">Compliance</Link>
          </div>
        </div>
      </div>
    </footer>
